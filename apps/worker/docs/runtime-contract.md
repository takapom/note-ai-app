...
- `wrangler.toml` は Cloudflare deployment entrypoint と Web static build artifact path を接続するだけにしてください。Turso URL/token、auth shared secret、workspace/user id、Note Model、Operation Router、AI provider policy を deployment config に直書きしないでください。
