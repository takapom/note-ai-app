...
import { createWorkerRuntimePorts } from '../composition/workerRuntimePorts.ts';
import {
  createLocalSmokeRuntimePorts,
  handleLocalSmokeVerificationRequest,
  handleLocalWorkspaceBrainProcessRequest,
} from '../composition/cloudflareVerificationWiring.ts';
...
export type { WorkerEntrypointEnv } from '../composition/workerEntrypointEnv.ts';
export { LOCAL_WORKSPACE_BRAIN_PROCESS_PATH } from '../composition/cloudflareVerificationWiring.ts';
...
export async function handleWorkerFetch<Env extends WorkerEntrypointEnv = WorkerEntrypointEnv>(
  ...
  if (env.LOCAL_AGENT_SMOKE_ENABLED === '1') {
    const localSmokeResponse = await handleLocalSmokeVerificationRequest(parsed.request, env);
    if (localSmokeResponse !== undefined) {
      return toFetchResponse(localSmokeResponse);
    }
  }
