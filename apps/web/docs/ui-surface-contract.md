...
- Browser runtime は `apiIntent: none` の expand/collapse/edit/cancel/close と、Worker block update 成功後の本文/heading title/idle 状態だけを UI projection として更新できます。失敗した save では browser projection を更新せず、canonical Note / Section / Block SoT の所有は Worker Note Model command boundary に残してください。
