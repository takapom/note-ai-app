...
test('structure job processor marks invalid generation runtime input as failed job', async () => {
  ...
  assert.equal(result.ok, false);
  assert.equal(result.reason, 'agent_failed');
  assert.equal(result.agent.orchestration.reason, 'invalid_runtime_input');
  assert.deepEqual(result.agent.errors, ['structure job status completed is not running']);
  assert.equal(workQueue.failedInputs.length, 1);
  assert.equal(workQueue.failedInputs[0].structureJobId, 'structure_job_processor_invalid_generation');
...
test('structure job processor preserves downstream audit failure while marking the job failed', async () => {
  ...
  assert.deepEqual(result.agent.orchestration.errors, [
    'audit operation_structure_job_processor_audit_failure_0: audit persistence failed: audit store unavailable',
  ]);
  assert.equal(result.failure.job.status, 'failed');
  assert.equal(
