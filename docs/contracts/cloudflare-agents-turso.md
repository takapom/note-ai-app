...
Worker -> NoteAgent/WorkspaceBrainAgent -> Operation Router -> audit persistence port -> Turso + AI SDK. Agent-local SQL は canonical ではない。
...
Runtime review は canonical-data violations、Agent-local SQL の逸脱、Turso Sync の混入、Operation Router を迂回した audit/application path を確認する。
