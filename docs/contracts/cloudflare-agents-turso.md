...
- Turso serverless client、Agent-local SQL client、auth shared secret、workspace/user identity は deployment-supplied Worker env/bindings、request headers、または runtime context から渡す。`wrangler.toml` はこれらの値を所有せず、repo-tracked public HTML も browser dataset metadata の実値や placeholder/sentinel tenant ID を所有しない。
