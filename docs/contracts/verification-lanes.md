...
- DOM host adapter guard: `node --test tests/contracts/web-note-surface-dom-host.test.mjs tests/contracts/web-note-surface-browser-runtime.test.mjs tests/contracts/web-note-surface-html-renderer.test.mjs`
  ...
  - verifies that active or just-ended input composition is represented on save descriptors as block-scoped DOM state, and browser runtime suppresses those saves before transport without clearing the editing/draft projection.
- browser runtime guard: `node --test tests/contracts/web-note-surface-browser-runtime.test.mjs tests/contracts/web-note-surface-event-controller.test.mjs tests/contracts/web-note-surface-html-renderer.test.mjs tests/contracts/web-note-surface-api-transport.test.mjs`
  - verifies that API-free UI actions (`expand_digest`, `collapse_digest`, `edit_block`, `cancel_edit`, `close_provenance`) update browser runtime projection state and re-render through the injected host without transport, while save / AI / memory actions keep their explicit Worker request paths.
  - verifies that editor dirty/saving/saved/error status is stored as browser UI projection state; save start renders a fixed status region with the current draft text and stable block identity attributes, successful block update responses update only the browser UI projection for saved paragraph/heading text and close editing state, and failed saves keep editing plus current draft projection with an error message and same-path `save_block` retry affordance without rewriting canonical Note / Section / Block SoT in the Web layer.
...
- hosted NoteSurface contract E2E guard: `node --test tests/contracts/hosted-note-surface-e2e.test.mjs`
  ...
  - verifies default Worker env binding wiring by passing fake `TURSO` and `AGENT_LOCAL_SQL` bindings through `createWorkerFetchHandler()` without `createPorts`, proving hosted artifact requests reach canonical Note SQL and Agent-local digest SQL through deployment-supplied bindings.
