...
- browser runtime guard: `node --test tests/contracts/web-note-surface-browser-runtime.test.mjs tests/contracts/web-note-surface-event-controller.test.mjs tests/contracts/web-note-surface-html-renderer.test.mjs tests/contracts/web-note-surface-api-transport.test.mjs`
  ...
  - verifies that editor dirty/saving/saved/error status is stored as browser UI projection state; save start renders a fixed status region with the current draft text, successful block update responses update only the browser UI projection for saved paragraph/heading text and close editing state, and failed saves keep editing plus current draft projection with an error message and same-path `save_block` retry affordance without rewriting canonical Note / Section / Block SoT in the Web layer.
