...
- browser runtime guard: `node --test tests/contracts/web-note-surface-browser-runtime.test.mjs tests/contracts/web-note-surface-event-controller.test.mjs tests/contracts/web-note-surface-html-renderer.test.mjs tests/contracts/web-note-surface-api-transport.test.mjs`
  ...
  - verifies that successful block update responses update only the browser UI projection for saved paragraph/heading text and close editing state, while failed saves leave the current editing projection intact and do not rewrite canonical Note / Section / Block SoT in the Web layer.
