...
apps/web emits UI events -> apps/worker route handlers -> Cloudflare Agents -> runtime operation routing adapter -> Operation Router -> audit persistence port -> Turso。  
...
runtime tests は note leave -> job enqueue、next open -> digest、manual organize -> operation generation request、AI response -> Operation Router -> audit persistence port の流れを検証しなければならない。
