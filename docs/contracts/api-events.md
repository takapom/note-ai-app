...
- UI events は `BlockChanged`, `NoteClosed`, `TabSwitched`, `AppLeft`, `NextOpen`, `ManualOrganizeRequested`, `AssistBlockEdited`, `AssistBlockDeleted`, `AssistBlockSourceInspected`, `MemoryCandidateAccepted`, `MemoryCandidateRejected` を含む。
...
- `POST /notes/:noteId/leave` は page/app leave のために、同じ request body に latest user-authored block updates を任意で含めてよい。Runtime はそれらを Note Model の explicit `block.update` command boundary と同じ検証・保存経路で順に保存し、すべて成功した場合だけ note leave scheduler trigger へ進む。save failure、invalid latest block update、active composition による frontend skip では provider、Operation Router、audit persistence、StructureJob enqueue に進んではならない。
...
- `OperationApplied` は canonical Note / Section / Block SoT の直接更新を意味しない。MVP の AI structuring path では silent effects は active projection persistence、inline/review effects は proposal persistence に限定され、整理結果は next-open digest または必要時の整理由来 projection として表示される。user-authored Block の変更は明示的な user edit または Note Model 所有 boundary を経由する。
