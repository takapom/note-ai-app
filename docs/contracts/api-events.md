...
- `OperationApplied` は canonical Note / Section / Block SoT の直接更新を意味しない。MVP の AI structuring path では silent effects は active projection persistence、inline/review effects は proposal persistence に限定され、AI Assist Block はユーザーの個別承認を必須にせず inline に表示されてよい。user-authored Block の変更は明示的な user edit または Note Model 所有 boundary を経由する。
