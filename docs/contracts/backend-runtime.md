...
- Runtime Memory Review port は `POST /memory/:memoryId/accept` と `POST /memory/:memoryId/reject` の delegated boundary であり、workspaceId、userId、memoryId、now を検証してから source-backed `memory_items` candidate/pending status と review metadata だけを更新する。invalid primitive では persistence に触れてはならず、source provenance、memory content、canonical Note/Section/Block、provider、Operation Router、Context Assembly、audit persistence を変更または呼び出してはならない。
