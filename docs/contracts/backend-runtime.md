...
Web client -> Worker API -> Agents -> completed StructureJob response -> runtime operation routing adapter -> Operation Router -> audit persistence port -> audit SQL adapter -> Turso operation audit executor -> Turso / AI SDK. Audit persistence failure -> operation audit recovery queue port.
