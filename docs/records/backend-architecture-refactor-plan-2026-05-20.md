...
- `runtime/cloudflare`
  ...
  - `cloudflareAgentBindings.ts` は binding descriptor / public RPC shape / Cloudflare-facing type / delegate injection surface に限定する。default NoteAgent / WorkspaceBrainAgent delegate 実装は `runtime/composition/agentDelegates.ts` が所有し、`runtime/cloudflare` から `scheduler` や `ai-operations/structure-job` を直接 import しない。
- `runtime/composition`
  - deployment-supplied env / bindings から runtime ports を組み立てる wiring（Step 3 でファイル分割、barrel なし）。
  - Cloudflare Agent default delegates の wiring を所有し、Cloudflare edge adapter と runtime context flows を接続する。
...
runtime/cloudflare
  ...
  → （禁止）note-model, scheduler, context-assembly, memory, ai-operations, contexts/*

runtime/composition
  → note-model / scheduler / context-assembly / memory / ai-operations（adapter・port factory のみ）
  → runtime/cloudflare（`cloudflareAgentBindings.ts` の descriptor / delegate surface、`cloudflareAgentRpcBoundary.ts` の RPC wiring port のみ）
...
| `cloudflareAgentBindings.ts` | `runtime/cloudflare/cloudflareAgentBindings.ts`（binding descriptor / public RPC shape / delegate injection surface のみ） |
| `cloudflareAgentBindings.ts` の default delegate 実装 | `runtime/composition/agentDelegates.ts`（production default delegates） |
...
| `runtime/composition/agentDelegates.ts` | default NoteAgent / WorkspaceBrainAgent delegates wiring |
...
- Step 2 では product semantics / HTTP・RPC の observable behavior は変えない。
...
- `cloudflareAgentBindings.ts` から default delegate 実装を `runtime/composition/agentDelegates.ts` へ移し、Cloudflare binding descriptor と runtime flow wiring を分離する。export 名と public RPC 型は維持する。
  - `NoteAgent` / `WorkspaceBrainAgent` は delegate injection surface を維持する。
  - production の zero-config default delegate が必要な factory は `runtime/composition/agentDelegates.ts` から supplied defaults を受ける。`cloudflareAgentBindings.ts` は context folders を直接 import しない。
- relative imports、tests import paths、`wrangler.toml` main path を更新する。
- path 更新漏れを避けるため、少なくとも次の import 元を明示確認する。
  - `runtime/cloudflare/cloudflareAgentBindings.ts`
  - `runtime/cloudflare/cloudflareAgentRpcBoundary.ts`
  - `runtime/http/workerHttpRouter.ts`
...
- 元の `apps/worker/src/scheduler/noteStructureRouteHandler.ts` は残っていない。
- `runtime/cloudflare/cloudflareAgentBindings.ts` は `scheduler` / `context-assembly` / `ai-operations` を直接 import していない。
...
- `runtime/composition`:
  ...
  - `agentDelegates.ts` で default NoteAgent / WorkspaceBrainAgent delegate wiring を明示し、`cloudflareAgentBindings.ts` から direct context import を除去した状態を維持する。
...
- `scripts/worker-local-smoke/*` へ smoke helper を分割し、`scripts/smoke-worker-local-runtime.mjs` は CLI orchestration entrypoint のみを残す。
...
- import guard はフォルダ単位だけでなく path prefix / file allowlist で判定する。
  - `runtime/cloudflare/**` から `contexts/*` への import を禁止する（live contract は各 context runtime 層が消費する）。
  - `scheduler -> ai-operations` は `ai-operations/structure-job` を含めて禁止する。
  - `ai-operations/*.ts`（`ai-operations/structure-job/**` 以外）から `scheduler` import を禁止する。
  - `ai-operations/structure-job/**` から `scheduler` への import は `scheduler/structureJobWorkQueuePort.ts` のみ許可する。
  - `runtime/composition/**` から `runtime/cloudflare/**` への import は `cloudflareAgentBindings.ts` と `cloudflareAgentRpcBoundary.ts` のみ許可する。
  - `runtime/cloudflare/cloudflareAgentBindings.ts` から context folders への direct import を禁止し、delegate wiring は `runtime/composition/agentDelegates.ts` に限定する。
- 残す regression tests:
