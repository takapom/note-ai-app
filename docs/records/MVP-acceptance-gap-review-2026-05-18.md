...
- browser deployment embedding adapter と browser app entry deployment bootstrap 後も残る blocking gap は framework package selection、bundler/build artifact serving path、actual deployment config、hosted page から Worker endpoint へ到達する E2E、deployment environment values の管理。
