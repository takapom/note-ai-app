...
| # | 条件 | 現状 | 根拠 / gap |
| --- | --- | --- | --- |
| 1 | ユーザーが一枚のノートに自然に書ける | covered | dependency-free AppShell / NoteSurface / Block Editor view model、HTML rendering、browser runtime、DOM host、explicit save click -> Worker request wiring、API-free browser projection actions、successful save 後の browser UI projection 更新、dirty/saving/saved/error status と same-path retry affordance、repo-local hosted contract E2E、composition 中 save suppression、focused block identity / draft text / fixed status region DOM hooks、DOM host Selection snapshot/restore、public artifact reserved status CSS、real browser editor guard は追加済み。Chrome-capable 環境で `node --test tests/contracts/web-note-surface-real-browser-editor.test.mjs` が 2026-05-19 に pass。 |
| 2 | H1/H2/H3 が section boundary として扱われる | covered | note-model document validation、web NoteSurface view model / renderer、explicit heading save click -> `PATCH /blocks/:blockId`、Worker Note Model command boundary での heading block text + owning section title/contentHash 同時更新、successful heading save 後の browser UI projection 更新、dirty/saving/saved/error status と fixed status region、repo-local hosted contract E2E、composition 中 save suppression、focused block identity DOM hooks、DOM host Selection snapshot/restore、public artifact reserved status CSS、real browser editor guard は追加済み。Chrome-capable 環境で `node --test tests/contracts/web-note-surface-real-browser-editor.test.mjs` が 2026-05-19 に pass。 |
| 3 | blocks と sections が内部正本として保存される | covered | canonical Note document persistence port / SQL adapter、block CRUD command port、HTTP router delegation、Worker fetch default Turso wiring、auth/workspace boundary、Cloudflare Agent binding foundation、wrangler deployment config、Web explicit save click -> `PATCH /blocks/:blockId` wiring、deployment environment values の repo-tracked 直書き禁止 contract / guard、repo-local hosted contract E2E、default Worker env binding E2E で検証済み。Agent-local save intent は canonical SoT ではない。 |
| 4 | note close / tab switch / app leave で dirty section の structure job が作られる | covered | scheduler domain、worker route handler cause preservation、HTTP router delegation、Worker fetch entrypoint、framework-neutral NoteAgent binding foundation、descriptor-derived Durable Object binding record、Worker main Agent class exports、wrangler Durable Object bindings/migration、default Worker env binding E2E、production auth verifier seam で検証済み。Production auth integration は repo-owned exact vendor ではなく deployment-owned verifier seam として固定済み。 |
...
| 9 | AI Assist Block が同じノート内に表示される | covered | Web NoteSurface view model、inline AI Assist action intents、Worker request descriptor mapping、fetch-like transport、framework-neutral HTML renderer、event controller、browser runtime、DOM host、action input resolver、app bootstrap、document-derived resolver options、product state composition、product app entrypoint、HTTP product provider、HTTP product app composition guard で検証済み。`GET /notes/:noteId` response の optional projection snapshot は Web が operation IDs を生成せず product state input に写す。browser deployment embedding adapter は global `document` / `fetch`、mount target lookup、deployment-supplied root dataset metadata を `browserNoteSurfaceMount.ts` に閉じる責務であり、browser app entry は injected runtime で mount adapter を起動する deployment bootstrap である。TypeScript browser ESM build artifact path と public deployment template、repo-local hosted contract E2E、wrangler static artifact / Worker route wiring、deployment environment values guard で検証済み。 |
| 10 | Next Open Digest が表示できる | covered | digest preparation、read boundary、HTTP router delegation、Worker fetch Agent-local wiring、Web compact/expandable view model、digest GET descriptor mapping、fetch-like transport、HTML renderer、event controller、browser runtime、DOM host、action input resolver、app bootstrap、document-derived resolver options、product state composition、product app entrypoint、HTTP product provider、HTTP digest product provider boundary、HTTP product app composition guard、HTTP digest product app composition guard、repo-local hosted contract E2E で検証済み。browser runtime は `digest.read` 成功 response body 由来の `nextOpenDigest` projection を UI state に反映して再描画し、failure では既存 projection を保持する。`GET /notes/:noteId` response/caller supplied `nextOpenDigest` は Web が生成・推測せず viewState に渡す。 |
| 11 | Memory candidate をノート内で承認または拒否できる | covered | Memory review port / SQL adapter / HTTP router / Worker fetch wiring、`create_memory_candidate` proposal 変換 boundary、Worker accept route/default Turso wiring、Web Memory Candidate action model、remember/edit/different/delete/hold descriptor mapping、fetch-like transport、HTML renderer、event controller、browser runtime、DOM host、action input resolver、app bootstrap、document-derived resolver options、product state composition、product app entrypoint、HTTP product provider、HTTP product app composition guard、repo-local hosted contract E2E で検証済み。browser runtime は memory remember/reject/delete/snooze 成功後に該当 candidate block を UI projection から非表示にし、memory.edit 成功後に response content を表示 text に反映し、failure では既存 projection を保持する。 |
| 12 | Provenance Popover で source を確認できる | covered | Provenance lookup port / SQL read adapter、`POST /provenance/source` Worker route / runtime wiring、Web bounded popover view model、AI Assist `Why?` / `inspect_source` -> `provenance.lookup` request descriptor mapping、caller supplied provenance resolver、fetch-like transport、HTML renderer、event controller、browser runtime、DOM host、action input resolver、app bootstrap、document-derived resolver options、product state composition、product app entrypoint、HTTP product provider、HTTP product app composition guard、repo-local hosted contract E2E で検証済み。browser runtime は実 UI の AI Assist event から `provenance.lookup` 成功 response body 由来の bounded excerpt / source metadata を provenance popover に反映して open にする。Web は provenance / sourceSpan IDs を生成・推測せず caller supplied mapping だけを使い、incomplete / invalid provenance mapping では request しない。 |
| 13 | AI provider failure が発生しても note editing は継続できる | covered | backend guard と web view model の failed AI status / editing action separation、browser editor save request path、API-free browser projection actions、failed save 時に editing + current draft projection を維持して error message / retry affordance を出す browser runtime guard、repo-local hosted contract E2E、composition 中 save suppression、focused block identity / draft text / fixed status region DOM hooks、DOM host Selection snapshot/restore、public artifact reserved status CSS、real browser editor guard は covered。Chrome-capable 環境で `node --test tests/contracts/web-note-surface-real-browser-editor.test.mjs` が 2026-05-19 に pass。 |
| 14 | MVP 除外 UI / 連携が入っていない | covered | Web view model、renderer validation、browser static build guard、integration guard が MVP-excluded surfaces、provider shortcut、user-authored block direct mutation を検出する。 |
| 15 | Codex task、Superset workspace、docs contract の traceability が維持される | partial | contracts/records は維持。GitHub issue close/create と最新 record commit の push は sandbox で未実行。 |
...
- worker auth/workspace boundary は request header/env/context または injected deployment-owned verifier から stable workspace/user identity を正規化し、configured shared secret mismatch や invalid verifier result では port factory 前に止める。repo-owned code は exact auth provider/JWT package を固定しない。
...
- Browser runtime は save 開始時に current draft text と `saving` status を UI projection として保持し、successful `block.update` response 後に saved paragraph / heading text と heading section title を UI projection として更新し、editing state を idle / saved に戻す。transport/controller failure では現在の editing + draft projection を保ち、error message と same-path `save_block` retry affordance を出す。
- Renderer は editable block ごとに stable block identity / fixed status region の DOM hooks（`data-editor-layout-stability="block-identity"`、`data-editor-status-region="fixed"`、`data-editor-layout-stability="status-reserved"`、`aria-live="polite"`、retry data attributes）を出し、status 表示の固定領域を browser guard で検証するための前提を contract test で固定済み。
- DOM host は save click で same-block `textContent` を読み続け、composition 中または composition end 後 input flush 前の save descriptor に `focusedBlockId` / `inputCompositionState` を付与する。Browser runtime は composition active / pending save を transport に送らず、editing / draft projection を変更しない。
- DOM host は HTML 再描画前後に focused editable block の Selection API text offset を snapshot/restore し、Browser runtime / renderer / domain model へ browser selection state を漏らさない。Public browser artifact は reserved status region / stable block layout の最小 CSS を持つ。`tests/contracts/web-note-surface-real-browser-editor.test.mjs` は Chrome DevTools Protocol で cursor offset と status region height を確認する guard として追加済み。Chrome-capable 環境で 2026-05-19 に pass。
...
- wrangler deployment config は `main = "apps/worker/src/runtime/http/workerEntrypoint.ts"`、`[assets].directory = "./dist/web"`、Worker-first API route patterns、Durable Object bindings、migration class list を固定する volatile deployment detail として追加済み。deployment environment values は `docs/contracts/backend-runtime.md` / `docs/contracts/cloudflare-agents-turso.md` に追記し、`tests/contracts/deployment-environment-values.test.mjs` で `wrangler.toml`、public HTML、browser dataset keys、Worker env interface を guard する。browser deployment embedding adapter、browser app entry deployment bootstrap、browser static build artifact path 後の hosted page から Worker endpoint への repo-local contract E2E と default Worker env binding E2E は `tests/contracts/hosted-note-surface-e2e.test.mjs` で追加済み。
...
1. GitHub issue close/create による traceability projection 更新。

## Review Notes

- MVP implementation acceptance #1-#14 は covered。MVP complete は #15 traceability projection 完了まで未宣言。repo-local hosted E2E、deployment environment values の repo-tracked 直書き禁止、Chrome-capable real browser editor guard は contract / focused guard として固定済み。
- DDD 境界上、deployment/auth integration は injected verifier と stable identity normalization に留め、product/domain policy と混ぜない。
- UI slice は残りの production editor polish に絞り、canonical Note / Section / Block SoT と Memory lifecycle policy を Web runtime に移さない。
- GitHub issue close/create と最新 record commit の push は sandbox policy により未実行。ユーザー側で traceability projection を更新する。
