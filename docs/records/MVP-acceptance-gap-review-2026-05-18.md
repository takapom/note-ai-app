...
| # | 条件 | 現状 | 根拠 / gap |
| --- | --- | --- | --- |
| 1 | ユーザーが一枚のノートに自然に書ける | partial | dependency-free AppShell / NoteSurface / Block Editor view model、HTML rendering、browser runtime、DOM host、explicit save click -> Worker request wiring、repo-local hosted contract E2E は追加済み。残りは richer editor interaction。 |
| 2 | H1/H2/H3 が section boundary として扱われる | partial | note-model document validation、web NoteSurface view model / renderer、explicit heading save click -> `PATCH /blocks/:blockId`、Worker Note Model command boundary での heading block text + owning section title/contentHash 同時更新、repo-local hosted contract E2E は追加済み。残りは richer editor interaction。 |
...
| 13 | AI provider failure が発生しても note editing は継続できる | partial | backend guard と web view model の failed AI status / editing action separation、browser editor save request path、repo-local hosted contract E2E は covered。残りは richer editor UX。 |
...
- repo-local hosted contract E2E は追加済み。残りは richer editor UX。
