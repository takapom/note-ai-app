# 生成ドキュメントレジスター

ドキュメント種別: 生成。マシン所有の証跡です。手動で編集してはいけません。

## コントラクト

- `docs/contracts/ai-structuring-lifecycle.md`
- `docs/contracts/api-events.md`
- `docs/contracts/app-note-model.md`
- `docs/contracts/authority-graph.md`
- `docs/contracts/backend-runtime.md`
- `docs/contracts/change-ownership.md`
- `docs/contracts/cloudflare-agents-turso.md`
- `docs/contracts/context-assembly.md`
- `docs/contracts/data-model.md`
- `docs/contracts/documentation-system.md`
- `docs/contracts/frontend-ui.md`
- `docs/contracts/memory.md`
- `docs/contracts/mvp-acceptance.md`
- `docs/contracts/mvp-scope.md`
- `docs/contracts/non-functional-requirements.md`
- `docs/contracts/operation-return-contract.md`
- `docs/contracts/product-principles.md`
- `docs/contracts/repository-topology.md`
- `docs/contracts/security-privacy.md`
- `docs/contracts/sot-and-projection.md`
- `docs/contracts/superset-codex-workflow.md`
- `docs/contracts/transitional-seams.md`
- `docs/contracts/unified-note-surface.md`
- `docs/contracts/vendor-lock-avoidance.md`
- `docs/contracts/verification-lanes.md`

## ガイド

- `docs/guides/codex-tasking-guide.md`
- `docs/guides/context-assembly-guide.md`
- `docs/guides/feature-development-guide.md`
- `docs/guides/implementation-readiness-guide.md`
- `docs/guides/operation-schema-guide.md`
- `docs/guides/review-guide.md`
- `docs/guides/subagent-routing-guide.md`
- `docs/guides/superset-mcp-workflow-guide.md`

## ランブック

- `docs/runbooks/agent-workspace-recovery.md`
- `docs/runbooks/ai-operation-regression.md`
- `docs/runbooks/contract-drift-recovery.md`

## 記録

- `docs/records/ADR/ADR-0001-ai-native-note-target-model.md`
- `docs/records/ADR/ADR-0002-ai-structuring-trigger.md`
- `docs/records/ADR/ADR-0003-block-section-document-model.md`
- `docs/records/ADR/ADR-0004-bounded-context-map.md`
- `docs/records/MVP-acceptance-gap-review-2026-05-18.md`
- `docs/records/MVP-acceptance-review-2026-05-28.md`
- `docs/records/README.md`
- `docs/records/backend-architecture-refactor-plan-2026-05-20.md`
- `docs/records/backend-ddd-hardening-issues-2026-05-19.md`
- `docs/records/backend-readiness-review-2026-05-19.md`
- `docs/records/change-plan/valibot-adoption-review-2026-05-23.md`
- `docs/records/frontend-architecture-implementation-plan-2026-05-20.md`
- `docs/records/frontend-ui-visual-direction-2026-05-21.md`
- `docs/records/local-cloudworker-agents-issues-2026-05-19.md`
- `docs/records/local-cloudworker-agents-smoke-handoff-2026-05-20.md`

## スキル

`.agents/skills/**/SKILL.md` を参照してください。

## サブエージェントブリーフ

`agents/subagents/*.md` を参照してください。
