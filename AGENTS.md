...
MVP の中心ループは `write -> leave note -> background structure -> next open digest -> editable AI assist blocks` です。永続的な AI チャットパネル、AI モード切り替え、外部連携、毎キー入力の AI 構造化、Markdown を内部 SoT にする設計は MVP 外です。
...
- 外部連携、AI chat、AI mode switcher、Markdown SoT など MVP 外の追加。
